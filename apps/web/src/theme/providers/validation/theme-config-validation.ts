...
import { StorageValidation, ThemeValidationError, TransitionValidation } from ".";
...
export class ThemeConfigValidation {
  ...
  static validateThemeConfig(config: ThemeInitOptions): ValidationResult {
    const { defaultThemeName: defaultScheme, defaultMode, storage, transition } = config;
    const errors: ThemeValidationError[] = [];

    // Validate defaultScheme
    if (defaultScheme && !(this.VALID_COLOR_SCHEMES as readonly string[]).includes(defaultScheme)) {
    ...
    if (storage) {
      ...
      if (!storageResult.isValid && storageResult.errors) {
        errors.push(...storageResult.errors.map(error =>
          new ThemeValidationError(error.message, error.code, error.path, error.details)
        ));
      }
    }
    ...
    // Validate transition
    if (transition) {
      const transitionResult = TransitionValidation.validateTransitionConfig(transition);
      if (!transitionResult.isValid && transitionResult.errors) {
        errors.push(...transitionResult.errors.map(error =>
          new ThemeValidationError(error.message, error.code, error.path, error.details)
        ));
      }
    }
