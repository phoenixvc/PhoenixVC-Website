...
export class ColorMapping {
      ...
      if (color !== undefined) {
        ...
        try {
          ...
          if (typeof color === "string") {
            ...
            colorDef = ColorUtils.ensureColorDefinition(
              color as Partial<ColorDefinition>,
            );
          }
          ...
          console.warn(
            `Invalid color value for ${path}, skipping: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
    });
  ...
  getShades(name: string): ColorShades | undefined {
    ...
    return Object.keys(shades).length === 10
      ? (shades as ColorShades)
      : undefined;
  }
    ...
    Object.entries(variables).forEach(([key, value]) => {
      if (key.startsWith(`--${this.config.prefix}`)) {
        const path = key.replace(
          `--${this.config.prefix}${this.config.separator}`,
          "",
        );
  ...
  adjustHue(color: ColorDefinition, degrees: number): ColorDefinition {
    ...
    const hslMatch = color.hsl.match(
      /hsla?\((\d+),\s*(\d+)%,\s*(\d+)%(?:,\s*([\d.]+))?\)/,
    );
    if (!hslMatch) {
  ...
  getContrastColor(color: ColorDefinition): ColorDefinition {
    ...
    if (color.rgb.startsWith("rgb")) {
      const rgbMatch = color.rgb.match(
        /rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([\d.]+))?\)/,
      );
      if (!rgbMatch) {
