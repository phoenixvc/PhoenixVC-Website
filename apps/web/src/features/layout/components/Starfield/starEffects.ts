...
// Single Responsibility: Star glow, hover effects, flares, nebula, and connections
...
export { drawSatellites } from "./satelliteRendering";

/**
 * Draw star glow and core with pulsing effect
 */
...
/**
 * Draw hover/select effects including pulsing rings and skill icons
 */
...
/**
 * Draw star flares - occasional bright flashes radiating from stars
 */
export function drawStarFlares(
  ...
  if (planet.pathType === "star" && Math.random() < 0.002) {
    ctx.save();
    const flareCount = 2 + Math.floor(Math.random() * 2);
    const flareLength = starSize * (1.8 + Math.random() * 2.2);
...
/**
 * Draw nebula effects - subtle background glow for important stars
 */
...
export function drawConnections(
  ...
  if (relatedStars.length > 0) {
    ...
    relatedStars.forEach(relatedStar => {
      ...
      ctx.beginPath();
      ctx.arc(midX, midY, 1.5, 0, TWO_PI);
      ctx.fillStyle = `rgba(${softRgb.r}, ${softRgb.g}, ${softRgb.b}, 0.4)`;
      ctx.fill();
    });
...
/**
 * Helper function to get project skills as an array
 */
