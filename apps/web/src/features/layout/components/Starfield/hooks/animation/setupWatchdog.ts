...
export const setupWatchdog = (
  ...
  refs.animationWatchdogRef.current = window.setInterval(() => {
    ...
    if (
      now - refs.lastFrameTimeRef.current > 3000 &&
      refs.isAnimatingRef.current &&
      !refs.isRestartingRef.current
    ) {
      logger.warn(
        `Animation appears frozen (${now - refs.lastFrameTimeRef.current}ms since last frame). Attempting restart...`,
      );
      ...
      if (refs.animationErrorCountRef.current < 5) {
        ...
        logger.error(
          "Too many animation restarts attempted. Animation disabled to prevent browser issues.",
        );
