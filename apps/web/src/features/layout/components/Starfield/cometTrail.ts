...
export function drawStarTrail(
  ...
  const dx =
    -fastSin(planet.angle) * (planet.orbitalDirection === "clockwise" ? 1 : -1);
  const dy =
    fastCos(planet.angle) * (planet.orbitalDirection === "clockwise" ? 1 : -1);
  ...
  const startWidth =
    COMET_CONFIG.trailWidthMultiplier * planetSize * scaleFactor;
  ...
  ctx.beginPath();
  ctx.moveTo(
    planet.x + (perpDx * startWidth) / 2,
    planet.y + (perpDy * startWidth) / 2,
  );

  // Create smooth curved trail path
  const cp1x =
    planet.x -
    normalizedDx * trailLength * 0.25 +
    (perpDx * startWidth) / 3 +
    wobbleAmount;
  const cp1y =
    planet.y - normalizedDy * trailLength * 0.25 + (perpDy * startWidth) / 3;
  ...
  ctx.lineTo(
    planet.x - normalizedDx * trailLength * COMET_CONFIG.coreLengthFraction,
    planet.y - normalizedDy * trailLength * COMET_CONFIG.coreLengthFraction,
  );
  ...
  for (let i = 0; i < debrisCount; i++) {
    ...
    const debrisX =
      planet.x -
      normalizedDx * distanceAlongTrail +
      perpDx * spreadX +
      spreadY * 0.3;
    const debrisY =
      planet.y -
      normalizedDy * distanceAlongTrail +
      perpDy * spreadX +
      spreadY * 0.3;
    ...
    if (debrisSize > 0.2 && debrisOpacity > 0.03) {
      ...
      debrisGlow.addColorStop(
        0.3,
        debrisRgbaPrefix + debrisOpacity * 0.6 + ")",
      );
  ...
  for (let i = 0; i < sparkleCount; i++) {
    ...
    const sparkleX =
      planet.x -
      normalizedDx * distanceAlongTrail +
      perpDx * sparkleSeed * sparkleSpread;
    const sparkleY =
      planet.y -
      normalizedDy * distanceAlongTrail +
      perpDy * sparkleSeed * sparkleSpread;
    ...
    sparkleGradient.addColorStop(
      0.2,
      sunRgbaPrefix + sparkleOpacity * 0.75 + ")",
    );
  ...
  for (let i = 0; i < jetCount; i++) {
    ...
    const jetDirX =
      normalizedDx * fastCos(jetAngle) - perpDx * fastSin(jetAngle);
    const jetDirY =
      normalizedDy * fastCos(jetAngle) - perpDy * fastSin(jetAngle);

    const jetLength =
      trailLength * (0.3 + fastSin(time * 0.0008 + i * 2) * 0.1);
