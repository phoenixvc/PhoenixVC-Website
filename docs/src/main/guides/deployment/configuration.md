...
### Environment-Specific Parameters {: #environment-specific-parameters}
```json
...
# Validate parameters using Bicep {: #validate-parameters-using-bicep}
...
```bash
# Check Key Vault access {: #check-key-vault-access}
az keyvault show \
  --name $KEYVAULT_NAME \
  --resource-group $RESOURCE_GROUP

# Verify NSG rules {: #verify-nsg-rules}
